...
//
// An exponential backoff approach is typically used when
// treating with potentially faulty/slow systems. If a system
// fails quick retries may exacerbate the system especially when the
// system is dealing with several clients.
//
// In this case a backoff provides the faulty system enough room
// for recovery.
//
// Simple example:
//
...
type Backoff interface {
	...
	// Reset clears the number of tries. Next call to NextAttempt will
	// return the minimum backoff time (if there is no error).
	Reset()
}
...
// Reset clears the number of attempts.
...
// lightly randomises the duration to minimise collisions between
// contending clients.
...
// Wait sleeps for the duration of the time specified by the
// backoff strategy.
...
// After returns a channel that will be called after the time
// specified by the backoff strategy or will exit immediately with an error.
